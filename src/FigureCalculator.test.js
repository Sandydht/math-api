...
describe('A FigureCalculator', () => {
  ...
  describe('A calculateRectanglePerimeter function', () => {
    ...
    it('should return correct value based on rectangle perimeter formula', () => {
      ...
      expect(spyAdd).toHaveBeenCalledWith(length, width); // length + width
      expect(spyMultiply).toHaveBeenCalledWith(2, (length + width)); // 2 * (length + width)
    });
  });
  ...
  describe('A calculateTriangleArea function', () => {
    ...
    it('should return correct value based on triangle area formula', () => {
      ...
      // Assert
      expect(result).toEqual(18); // (base * height) / 2
      expect(spyMultiply).toHaveBeenCalledWith(base, height); // base * height
      expect(spyDivide).toHaveBeenCalledWith((base * height), 2); // (base * height) / 2
    });
  });
});
